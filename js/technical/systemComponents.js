var systemComponents = {
	...
	'overlay-head': {
		...
		<h2  class="overlayThing" id="points">{{format(player.points.sub(player.ab.negativePoints))}}</h2>
		<span v-if="player.points.lt('1e1e6')"  class="overlayThing"> {{modInfo.pointsName}}</span>
		<br>
		<span v-if="canGenPoints()"  class="overlayThing">({{tmp.other.oompsMag != 0 ? format(tmp.other.oomps) + " OOM" + (tmp.other.oompsMag < 0 ? "^OOM" : tmp.other.oompsMag > 1 ? "^" + tmp.other.oompsMag : "") + "s" : formatSmall(getBypassedPointGen())}}/sec)</span>
