...
function getNextAt(layer, canMax=false, useType = null) {
	...
	if (type=="static") 
	{
		...
		if (!tmp[layer].canBuyMax) canMax = false
		let boosters = player[layer].points
		if (tmp[layer].layer=="b") boosters = boosters.plus(player.pb.boosters)
		if (tmp[layer].layer=="kb") boosters = boosters.plus(player.pb.kiloboosters)
		if (tmp[layer].layer=="mb") boosters = boosters.plus(player.pb.megaboosters)
		if (tmp[layer].layer=="gb") boosters = boosters.plus(player.pb.gigaboosters)
		let amt = boosters.plus((canMax&&tmp[layer].baseAmount.gte(tmp[layer].nextAt))?tmp[layer].resetGain:0).div(tmp[layer].directMult)
		let extraCost = Decimal.pow(tmp[layer].base, amt.pow(tmp[layer].exponent).div(tmp[layer].gainExp)).times(tmp[layer].gainMult)
		let cost = extraCost.times(tmp[layer].requires).max(tmp[layer].requires)
		if (tmp[layer].roundUpCost) cost = cost.ceil()
		return cost;
...
{	
	...
	else if(tmp[layer].type== ("static" || "scaling static"))
...
function doReset(layer, force=false) {
	...
	if (!force) {
		...
		if (tmp[layer].type==("static"||"scaling static")) {
...
function hardReset(resetOptions) {
	...
	options.assholeMode = false
	options.meSmart = false
	save(true);
