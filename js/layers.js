...
addLayer("s", {
	...
	spissitude() {let size = tmp.s.buyables[41].size
				  return size},
    gainMult() { // Calculate the multiplier for main currency from bonuses
        return new Decimal(1).mul(tmp.t.effectDay).mul(hasUpgrade("c", 23)?upgradeEffect("c", 23):1).mul(hasUpgrade("c", 43)?upgradeEffect("c", 43):1).mul(tmp.c.buyables["compress2"].effect2.pow(10)).mul(tmp.c.crescentEffect).mul(tmp.c.buyables["CB1"].effect)
    },
	update(diff){
		...
		if(hasMilestone("c","c1")){
			...
			if(tmp.s.buyables[21].canAfford) player.s.buyables[21]=player.s.buyables[21].add(1)
			if(tmp.s.buyables[31].canAfford) player.s.buyables[31]=player.s.buyables[31].add(1)
			if(tmp.s.buyables[41].canAfford) player.s.buyables[41]=player.s.buyables[41].add(1)
		}
	},
    gainExp() { // Calculate the exponent on main currency from bonuses
        return new Decimal(1)
...
addLayer("c", {
	...
	crescentGain(){return player.c.crescentmoonsTotal.add(1).root(29.5305888531).sub(1).mul(tmp.c.buyables["CB1"].effect2)},
    ...
    gainMult() { // Calculate the multiplier for main currency from bonuses
        ...
        return mult.mul(tmp.c.crescentEffect)
    },
    gainExp() { // Calculate the exponent on main currency from bonuses
        return new Decimal(1)
    },
	...
	buyables: {
		...
		CB3: {
			...
			effect(){return Decimal.pow(2.95305888531, player.c.buyables["CB3"])},
			effect2(){return player.c.buyables["CB3"].add(1).root(2.95305888531)},
			display(){return `Multiplies the other two Crescentables's effective amount by ${format(this.effect2())}x and your point gain is ${format(this.effect())}x stronger<br><br>Cost: ${formatWhole(this.cost())} crescent moons<br>Amount: ${formatWhole(player.c.buyables["CB3"])}`},
			cost(){return Decimal.pow(3, player.c.buyables["CB1"].add(player.c.buyables["CB2"]).add(player.c.buyables["CB3"]))},
			canAfford(){return player.c.crescentmoons.gte(this.cost())},
			buy(){
				player.c.crescentmoons=player.c.crescentmoons.sub(this.cost())
...
addLayer("realAB",{
	...
	buyables:{
		...
		21: {
			...
			canAfford() { return player.o.plots.gte(this.cost())&&player.realAB.points.gte(player.realAB.buyables[21].gte(3)?7:5) },
			buy() {
